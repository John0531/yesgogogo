<template>
  ...
  <div class="product" v-if="isEmpty === false">
    ...
    <div v-show="isLoading" >
      ...
      <template v-if="this.data.categoryList">

        <!-- v-if="this.data.categoryList || this.$store.state.subCategory !== '' || this.$store.state.category !=='' " -->

          <nav  aria-label="breadcrumb" class="d-flex mb-2">
            <ol class="breadcrumb mb-md-2 ms-md-0">
              ...
              <template v-if="this.data.categoryList.length>0 ">

              <li class="breadcrumb-item">
                <router-link to="/">首頁</router-link>
              </li>

              <router-link v-if="this.data.categoryList[0]" :to="{ path: `/productboard/productlist/${ this.data.categoryList[0].categoryId }`}" class="breadcrumb-item"
                @click.prevent="clearBreadcrumbName"
              >
                {{this.data.categoryList[0].categoryName }}
              </router-link>
              <router-link v-if="this.data.categoryList[0].subCategory[0]" :to="{ path: `/productboard/productlist/${this.data.categoryList[0].subCategory[0].categoryId}`}" class="breadcrumb-item">
                {{ this.data.categoryList[0].subCategory[0].categoryName }}
              </router-link>
            </template>
            </ol>
          </nav>
      ...
      <div class=" my-4">
        ...
        <img src="@/assets/img/yesgo_icon39.svg" alt="每日強打" class="subtitle-img hot-title top-0 translate-middle bg-white  py-1 py-lg-2 ps-1 ps-md-2">

  <div class="d-none d-md-block  pt-3 my-3 hotLine" v-if="historyData.length>0">
    <div class="swiper-custom-PC ">
          <swiper
                class="swiper-custom-wrap-PC  "

                :freeMode="true"
                :navigation="{
                nextEl: '#productHot-next',
                prevEl: '#productHot-prev'
                }"
              ...
              >
                <swiper-slide v-for="item in  productHot"  :key="item.productId">

                  <div class="card h-100 watchHistoryStyle bg-transparent border-0" >
                      <router-link :to="{ path: `/productboard/product/${item.productId}`}"  class="text-dark stretched-link "   >
                          <div class="imgContainer ratio ratio-1x1" >
                              <img :src="item.productImage" class="card-img-top" alt="...">
                          </div>
                      <div class="card-body px-0 pb-0">
                          <p class="card-title fs-7 card-title-custom-height"> {{ item.name  }}  </p>
                            <p class="card-text text-primary "><span class="fs-7">特價$</span>{{ $currency.currency(item.price)  }}  </p>
                      </div>
                      </router-link>
                  </div>
              ...
              <!-- navigation buttons -->
              <div id="productHot-prev" class="swiper-button-prev " >  </div>
              <div id="productHot-next" class="swiper-button-next "  >  </div>
      ...
      <!-- 手機版 每日強打  -->
      <div v-if="productHot" class="d-md-none bg-light px-2 pt-3 mt-4">
        ...
        >
          <swiper-slide v-for="item in productHot" :key="item.productId"><a href="#" @click.prevent="toHotProduct(item.productId);this.$store.commit('getSubCategoryId', '');" class="card-hover">
              <div class="mb-1 imgContainer ratio ratio-1x1">
                <img :src="item.productImage" class="card-img-top" alt="..." />
              </div>
              <small class="d-block text-dark fs-7 hotproduct-mobile-name card-title-custom-height" >{{ item.name }}</small>
              <small class="text-primary"><span class="fs-7">特價$</span>{{ item.price }}</small>
            </a>
          </swiper-slide>
        </swiper>
      </div>
 ...
 <!-- 他們也逛了-->
<div class="bg-light p-3 pb-1 my-3" v-if="otherWatchData.length>0" >
    <div class="d-flex justify-content-between align-items-end  ">
        <img src="@/assets/img/yesgo_otherWatch_icon.svg" alt="他們也逛了" class="subtitle-img py-1 py-lg-2 ps-1  ps-md-2">
    </div>

      <div class="position-relative swiper-custom-PC">
          <swiper
                class="swiper-custom-wrap-PC  "

                :freeMode="true"
                :navigation="{
                nextEl: '#otherWatch-next',
                prevEl: '#otherWatch-prev'
                }"
              ...
              >   saveWatchHistory()
                <swiper-slide v-for="otherWatch in  otherWatchData"  :key="otherWatch.id">

                  <div class="card h-100 watchHistoryStyle bg-transparent border-0" >
                      <router-link :to="{ path: `/productboard/product/${otherWatch.id}`}"  class="text-dark stretched-link "   >
                          <div class="imgContainer ratio ratio-1x1" >
                              <img :src="otherWatch.productImage" class="card-img-top" alt="...">
                          </div>
                      <div class="card-body px-0 pb-0">
                          <p class="card-title fs-7 card-title-custom-height"> {{ otherWatch.name  }}  </p>
                            <p class="card-text text-primary "> <span class="fs-7">$</span>{{ $currency.currency(otherWatch.price)  }}  </p>
                      </div>
                      </router-link>
                  </div>
              ...
              <!-- navigation buttons -->
              <div id="otherWatch-prev" class="swiper-button-prev d-none d-lg-block" >  </div>
              <div id="otherWatch-next" class="swiper-button-next d-none d-lg-block"  >  </div>

        </div>
</div>

 <!-- 瀏覽紀錄-->
<div class="bg-light p-3 pb-1 " v-if="historyData.length>0" >
    <div class="d-flex justify-content-between align-items-end  ">
        <img src="@/assets/img/yesgo_watchHistory_icon.svg" alt="瀏覽紀錄" class="subtitle-img py-1 py-lg-2 ps-1  ps-md-2">
    </div>
      <!-- 瀏覽紀錄 改用swiper   :slidesPerGroup="3" :pagination="{
                 type: 'progressbar',
                 dynamicBullets: true,
                 clickable: true,
                }" -->
      <div class="position-relative swiper-custom-PC">
          <swiper
                ...
                :freeMode="true"
                :navigation="{
                nextEl: '#watchRecord-next',
                prevEl: '#watchRecord-prev'
                }"
              ...
              >
                <swiper-slide v-for="history in  historyData"  :key="history.pathUrl">

                  <div class="card h-100 watchHistoryStyle bg-transparent border-0"  >
                      <router-link :to="{ path: `${history.pathUrl}`}"  class="text-dark stretched-link "   >
                          <div class="imgContainer ratio ratio-1x1" >
                              <img :src="history.picUrl" class="card-img-top" alt="...">
                          </div>
                      <div class="card-body px-0 pb-0">
                          <p class="card-title fs-7 card-title-custom-height"> {{ history.name  }}  </p>
                            <p class="card-text text-primary "> <span class="fs-7">$</span>{{ $currency.currency(history.price)  }}  </p>
                      </div>
                      </router-link>
                  </div>
              ...
              <!-- navigation buttons -->
              <div id="watchRecord-prev" class="swiper-button-prev  d-none d-lg-block" >  </div>
              <div id="watchRecord-next" class="swiper-button-next d-none d-lg-block"  >  </div>

        </div>
</div>

<!-- 猜你喜歡 yesgo_guessYouLike_icon -->
<!-- <div class="bg-light p-3 pb-1 my-3" v-if="historyData.length>0" >
    <div class="d-flex justify-content-between align-items-end  ">
        <img src="@/assets/img/yesgo_guessYouLike_icon.svg" alt="猜你喜歡" class="subtitle-img py-1 py-lg-2 ps-1  ps-md-2">
    </div>
      ...
      <div class="position-relative swiper-custom-PC">
          <swiper
                ...
                :freeMode="true"
                :navigation="{
                nextEl: '#guessYouLike-next',
                prevEl: '#guessYouLike-prev'
                }"
              ...
              >
                <swiper-slide v-for="history in  historyData"  :key="history.pathUrl">
                  ...
                  <div class="card h-100 watchHistoryStyle bg-transparent border-0" >
                      ...
                      <div class="card-body px-0 pb-0 ">
                          <p class="card-title fs-7 card-title-custom-height"> {{ history.name  }}  </p>
                            <p class="card-text text-primary "> NT$ {{ $currency.currency(history.price)  }}  </p>
                      </div>
                      </router-link>
                  </div>
              ...
              <div id="guessYouLike-prev" class="swiper-button-prev " >  </div>
              <div id="guessYouLike-next" class="swiper-button-next "  >  </div>
...
export default {
  ...
  methods: {
    ...
    //! 取得瀏覽紀錄
    getWatchHistory () {
      this.historyData = JSON.parse(localStorage.getItem('watchHistory')).reverse()
    },
    //! 若local尚未有watchHistory資料時跑這段
    createWatchHistory () {
      const watchTimestamp = (+new Date())

      const timeInterval = 60 * 60 * 24 * 1000 * 30 //! 30天時間的總毫秒數字
      // const timeInterval = 10 * 60 * 1000 //! 10分鐘過期(開發用)

      const invalidTimastamp = watchTimestamp + timeInterval

      const productData = {
        pathUrl: this.$route.path,
        name: this.data.name,
        price: this.data.price,
        picUrl: this.data.productImage,
        watchTimestamp: watchTimestamp,
        invalidTimastamp: invalidTimastamp
      }

      const recordArr = []
      recordArr.push(productData)
      // 存進local
      localStorage.setItem('watchHistory', JSON.stringify(recordArr))
    },
    //! 儲存瀏覽紀錄 限制只顯示最新的12筆
    saveWatchHistory () {
      // console.log('當前瀏覽紀錄商品id', this.$route.params.productId)
      // console.log('this.data', this.data)
      if (this.$route.params.productId !== this.data.productId) {
        return
      }

      //! 判斷local是否有存watchHistory資料
      if (localStorage.getItem('watchHistory')) {
        const recordArr = JSON.parse(localStorage.getItem('watchHistory'))
        const currentTimestamp = (+new Date())
        // console.log('當前timestamp', currentTimestamp)

        //! 先把過期的品項刪除
        const delArr = recordArr.reduce((acc, element, index) => {
          if (currentTimestamp > element.invalidTimastamp) { acc.push(index) }
          return acc
        }, [])

        delArr.sort((a, b) => b - a) //! 把要刪除的索引值delArr降冪排列，才不會影響recordArr順序
        delArr.forEach((delIndex) => { recordArr.splice(delIndex, 1) })

        // ! 要判斷對比this.$route.path和local裡的pathUrl 重複的不存
        if (recordArr.find((item) => item.pathUrl === this.$route.path)) {
          //! 找到二次點擊的商品index，(移除後，再push)更新到第一筆並重新計算效期
          const targetIndex = recordArr.findIndex((item) => item.pathUrl === this.$route.path)
          if (targetIndex !== -1) {
            const [remove] = recordArr.splice(targetIndex, 1)

            const timeInterval = 60 * 60 * 24 * 1000 * 30 //! 30天時間的總毫秒數字
            // const timeInterval = 10 * 60 * 1000 //! 10分鐘過期(開發用)

            remove.watchTimestamp = (+new Date())
            remove.invalidTimastamp = (+new Date()) + timeInterval

            recordArr.push(remove)
            localStorage.setItem('watchHistory', JSON.stringify(recordArr))
          }
        ...
        this.createWatchHistory()
        this.getWatchHistory()
      }
    },
    getOtherWatch () {
      const url = `${process.env.VUE_APP_API}/Api/Product/ClassifyProducts?productId=${this.productId}`
      this.axios.get(url)
        .then((res) => {
          if (res.data.rtnCode === 0) {
            const apiOtherWatchData = res.data.info.results

            const historyIdArr = this.historyData.map((item) => item.pathUrl.substring(item.pathUrl.lastIndexOf('/') + 1))

            this.otherWatchData = apiOtherWatchData.filter((item) => {
              return historyIdArr.every((id) => item.id !== id
              )
            })
          }
        })
...
// ::v-deep(.swiper-watchRecord-prev:after){
// content:'<' ;
// font-size: 1rem;
// color: #fff;
...
// ::v-deep(.swiper-watchRecord-next:after){
// content:'>' ;
// font-size: 1rem;
// color: #fff;
// background:rgba(0,0,0,0.3);
...
&:hover{
  color: #F8412E;
 border: 1px solid rgba(248, 65, 46, 0.6);
}
}
...
.swiper-custom-wrap-PC{
//  width:100%;
// margin:0 auto;
 @media (min-width: 576px) { //延續現行mb樣式
   width:97%;
   margin:0 auto;
   }
...
::v-deep(.swiper-custom-PC .swiper-button-next){
//   position: absolute;
//   right: 10px;
// @media (min-width: 768px) {
  position: absolute;
...
::v-deep(.swiper-custom-PC .swiper-button-prev){
//   position: absolute;
//   right: 10px;
// @media (min-width: 768px) {
  position: absolute;
...
::v-deep(.swiper-custom-PC .swiper-button-prev:after) , ::v-deep(.swiper-custom-PC .swiper-button-next:after) {
  padding: 10px ;
  color:  #999;
  font-size: 1rem;
