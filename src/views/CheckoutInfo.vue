                ...
                <div class="col-10 col-md-10">
                  <div class="row g-2 align-items-center">
                    <div class="col-6 col-md-2">
                      <Field
                        type="text"
                        v-model.trim="checkOrder.consignee.consigneeAreaNo"
                        name="區碼"
                        class="form-control"
                        id="receiverLocalPhone1"
                        placeholder="區碼"
                        maxlength="3"
                        @change="isLocalPhone1(checkOrder.consignee.consigneeAreaNo, $refs.disabledd, '區碼')"
                        :class="{ 'is-invalid': errors['區碼'] }"
                      ></Field>
                      <ErrorMessage name="區碼" class="invalid-feedback">
                      </ErrorMessage>
                    </div>
                    <div class=" text-center px-1 cardDash">
                    </div>
                    <div class="col-12 col-md-4">
                      <Field
                        type="text"
                        v-model.trim="checkOrder.consignee.consigneePhoneNo"
                        name="市內電話"
                        class="form-control"
                        id="receiverLocalPhone2"
                        placeholder="請填入市話或備用電話"
                        maxlength="8"
                        @change="isLocalPhone2(checkOrder.consignee.consigneePhoneNo, $refs.disabledd, '區碼')"
                        :class="{ 'is-invalid': errors['市內電話'] }"
                      ></Field>
                      <ErrorMessage name="市內電話" class="invalid-feedback">
                      </ErrorMessage>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="col-xl-4">
                    <div class="d-xl-flex align-items-center">
                      ...
                      <div class="d-flex justify-content-end align-items-center pe-0">
                        <button class="d-block btn btn-secondary  shadow-none  border-secondary text-nowrap ms-1 mt-2 mt-lg-0 fs-7 p-md-1 p-1 btn-sm " @click.prevent="checkTaxNumberExist">
                          帶入公司名
                        </button>
                      </div>
                    </div>
                ...
                <div class="col-10  mt-2 mt-md-0">
                  <ul >
                    <li class="d-flex">
                      ...
                      <span class="me-2">姓名:{{item.consigneeName}}</span></li>
                    <li class="d-flex">
                      <i class="bi bi-geo-alt-fill me-3"></i>
                      <span class="me-2">地址:{{item.cityName}}{{item.districtName}}{{item.otherAddress}}</span>
                    </li>
                    <li class="d-flex">
                      <i class="bi bi-telephone-fill me-3"></i>
                      <span class="me-2">手機:{{item.cellphone}}</span>
                    </li>
                    <li class="d-flex" v-if="item.consigneeAreaNo || item.consigneePhoneNo">
                      <i class="bi bi-telephone me-3 d-inline-block"></i>
                      <span class="me-2">市話:{{item.consigneeAreaNo}}-{{item.consigneePhoneNo}}</span>
                    </li>
                    <li class="d-flex">
                      <i class="bi bi-envelope-fill me-3 d-inline-block"></i>
                      <span class="me-2">電子郵件:{{item.consigneeEmail}}</span>
                    </li>
                  </ul>
                </div>
                <!-- <div class="col-10  mt-2 mt-md-0">
                  ...
                  <p class="text-secondary fs-6">
                    <i class="bi bi-geo-alt-fill me-3"></i>
                    <span class="me-2 contents">地址:{{item.cityName}}{{item.districtName}}{{item.otherAddress}}</span>
                  </p>
                  <p class="text-secondary fs-6"><i class="bi bi-telephone-fill me-3"></i>電話:<span class="me-2"></span>{{item.cellphone}}</p>
                  <p class="text-secondary fs-6">
                    <i class="bi bi-envelope-fill me-3 d-inline-block"></i>
                    <span class="contents">電子郵件:{{item.consigneeEmail}}</span>
                  </p>
                </div> -->
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    pickUsedContact (item) { // ?選擇常用聯絡人帶入
      this.$swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          this.checkOrder.consignee.consigneeAreaNo = item.consigneeAreaNo
          this.checkOrder.consignee.consigneePhoneNo = item.consigneePhoneNo
          // 重新產生鄉鎮清單
          this.addressList.townList = this.countryName.filter(
            item => item.Country === this.checkOrder.consignee.consigneeCity
          )
    ...
    addUsedContact () { // ? 加入常用聯絡人
      ...
      this.addUsedContactList.consigneeAreaNo = this.checkOrder.consignee.consigneeAreaNo
      this.addUsedContactList.consigneePhoneNo = this.checkOrder.consignee.consigneePhoneNo
    ...
    isLocalPhone1 (value, FormDom, fieldName) {
      FormDom.setFieldError(fieldName, '')
      const phoneNumber = /\d{2,3}/
      return phoneNumber.test(value) ? true : '請輸入2-3碼區碼格式'
    },
    isLocalPhone2 (value, FormDom, fieldName) {
      FormDom.setFieldError(fieldName, '')
      const phoneNumber = /\d{3,4}\d{4}/
      return phoneNumber.test(value) ? true : '請輸入7-8碼市話格式'
    },
